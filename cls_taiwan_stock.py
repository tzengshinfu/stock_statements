...
class ClsTaiwanStock():
    ...
    def get_basic_info_files(self, stock_list: List[NamedTuple('stock', [('id', str), ('name', str)])]):
        ...
        for stock in stock_list:
            book_path = self.__excel.books_path + '\\' + stock.id + '(' + stock.name + ')_基本資料' + '.xlsx'
    ...
    def get_statment_files(self, stock_list: List[NamedTuple('stock', [('id', str), ('name', str)])], top_n_seasons: int):
        ...
        def get_statment_file(stock: NamedTuple('stock', [('id', str), ('name', str)]), period: NamedTuple('period', [('year', str), ('season', str)]), table_type: str):
            book_path = self.__excel.books_path + '\\' + stock.id + '(' + stock.name + ')_{0}'.format(table_type) + '.xlsx'
            self.__excel.open_book(book_path)
    ...
    def get_analysis_files(self, stock_list: List[NamedTuple('stock', [('id', str), ('name', str)])], top_n_seasons: int):
        ...
        for stock in stock_list:
            for period in periods:
                ...
                self.__excel.write_to_sheet(list1)
                book_path = self.__excel.books_path + '\\' + stock.id + '(' + stock.name + ')_財務分析.xlsx'
                self.__excel.save_book(book_path)
                self.__current_process += 1
    ...
    def get_dividend_files(self, stock_list: List[NamedTuple('stock', [('id', str), ('name', str)])], top_n_seasons: int):
        ...
        for stock in stock_list:
            for period in periods:
                ...
                list1 = self.__to_list(table)
                self.__excel.write_to_sheet(list1)
                book_path = self.__excel.books_path + '\\' + stock.id + '(' + stock.name + ')_股利分派情形.xlsx'
                self.__excel.save_book(book_path)
                self.__current_process += 1
    ...
    def show_config_form(self) -> NamedTuple('result', [('action', str), ('drive_letter', str), ('directory_name', str), ('top_n_seasons', str)]):
        """開啟設定介面
            ...
            Returns:
                NamedTuple('result', [('action', str), ('drive_letter', str), ('directory_name', str)]) -- 執行動作/磁碟代號/目錄名稱
        """
        form = gui.FlexForm('設定台股上巿股票Excel存放路徑')
        layout = [[gui.Text('請輸入下載Excel存放的磁碟代號及目錄名稱')], [gui.Text('Drive', size=(15, 1)), gui.InputText('Z')], [gui.Text('Folder', size=(15, 1)), gui.InputText('Excel')], [gui.Text('請輸入下載前n季(0=不限)')], [gui.Text('Top', size=(15, 1)), gui.InputText('1')], [gui.Submit(), gui.Cancel()]]
    ...
    def show_running_process(self):
        ...
        layout = [[gui.Text('完成進度')], [gui.Text('完成進度', key='current_processing')], [gui.ProgressBar(self.__total_processes, orientation='h', size=(20, 20), key='progressbar')], [gui.Cancel()]]
        window = form.Layout(layout)
        while True:
            ...
            if self.__total_processes > 0 and self.__current_process > 0 and self.__total_processes == self.__current_process:
                break
            window.FindElement('progressbar').UpdateBar(str(self.__current_process))
            window.FindElement('current_processing').Update(str(self.__current_process) + '/' + str(self.__total_processes))
        window.Close()

    def set_total_processes(self, stock_list: List[NamedTuple('stock', [('id', str), ('name', str)])]):
        self.total_processes = len(stock_list) * self.__sheet_count
